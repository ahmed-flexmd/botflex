  ...
  try {
    ...
    try {
      ...
      if (gameglx) {
        ...
        if (!("recebidas" in gameglx.notificacao))
          gameglx.notificacao.recebidas = [];
        // Perfil
        ...
        if (!("nome" in gameglx.perfil.nivel))
          gameglx.perfil.nivel.nome = "Iniciante";
        if (!("id" in gameglx.perfil.nivel)) gameglx.perfil.nivel.id = 0;
        if (!("proximoNivel" in gameglx.perfil.nivel))
          gameglx.perfil.nivel.proximoNivel = 1;
        if (!("xp" in gameglx.perfil)) gameglx.perfil.xp = 112;
        if (!("ar" in gameglx.perfil)) gameglx.perfil.ar = "pt-br"; // Definindo padrão
        ...
        if (!("idpelonome" in gameglx.perfil.casa))
          gameglx.perfil.casa.idpelonome = "terra";
        if (!("planeta" in gameglx.perfil.casa))
          gameglx.perfil.casa.planeta = null;
        if (!("colonia" in gameglx.perfil.casa))
          gameglx.perfil.casa.colonia = null; // Definir como null em vez de objeto vazio
        if (gameglx.perfil.casa.colonia === null)
          gameglx.perfil.casa.colonia = {}; // Verificar se é null antes de definir como objeto vazio
        if (!("nome" in gameglx.perfil.casa.colonia))
          gameglx.perfil.casa.colonia.nome = null;
        if (!("id" in gameglx.perfil.casa.colonia))
          gameglx.perfil.casa.colonia.id = 1;
        if (!("habitante" in gameglx.perfil.casa.colonia))
          gameglx.perfil.casa.colonia.habitante = false;

        // Carteira Dinheiro
        if (!("carteira" in gameglx.perfil)) gameglx.perfil.carteira = {};
        if (!("currency" in gameglx.perfil.carteira))
          gameglx.perfil.carteira.currency = "BRL"; // Definindo padrão
        if (!("saldo" in gameglx.perfil.carteira))
          gameglx.perfil.carteira.saldo = 1500;
        // localizacao
        if (!("localizacao" in gameglx.perfil)) gameglx.perfil.localizacao = {};
        if (!("status" in gameglx.perfil.localizacao))
          gameglx.perfil.localizacao.status = false;
        if (!("nomeplaneta" in gameglx.perfil.localizacao))
          gameglx.perfil.localizacao.nomeplaneta = null;
        if (!("idpelonome" in gameglx.perfil.localizacao))
          gameglx.perfil.localizacao.idpelonome = null;
        if (!("viajando" in gameglx.perfil.localizacao))
          gameglx.perfil.localizacao.idpelonome = false;
        if (!("id" in gameglx.perfil.localizacao))
          gameglx.perfil.localizacao.id = null;
        //Posição  na casa Colonia
        if (!("posicao" in gameglx.perfil.casa.colonia))
          gameglx.perfil.casa.colonia.posicao = {};
        if (!("x" in gameglx.perfil.casa.colonia.posicao))
          gameglx.perfil.casa.colonia.posicao.x = 0;
        if (!("y" in gameglx.perfil.casa.colonia.posicao))
          gameglx.perfil.casa.colonia.posicao.y = 0;

        //Posição  em viagens se necessario
        if (!("posicao" in gameglx.perfil.localizacao))
          gameglx.perfil.localizacao.posicao = {};
        if (!("x" in gameglx.perfil.localizacao.posicao))
          gameglx.perfil.localizacao.posicao.x = 0;
        if (!("y" in gameglx.perfil.localizacao.posicao))
          gameglx.perfil.localizacao.posicao.y = 0;
        // nave
        if (!("nave" in gameglx.perfil)) gameglx.perfil.nave = {};
        if (!("nome" in gameglx.perfil.nave))
          gameglx.perfil.nave.status = false;
        if (!("id" in gameglx.perfil.nave)) gameglx.perfil.nave.id = null;
        if (!("nome" in gameglx.perfil.nave)) gameglx.perfil.nave.nome = null;
        if (!("velocidade" in gameglx.perfil.nave))
          gameglx.perfil.nave.velocidade = null;
        ...
        if (!("madeira" in gameglx.perfil.bolsa.itens))
          gameglx.perfil.bolsa.itens.madeira = 1;
        if (!("ferro" in gameglx.perfil.bolsa.itens))
          gameglx.perfil.bolsa.itens.ferro = 1;
        if (!("diamante" in gameglx.perfil.bolsa.itens))
          gameglx.perfil.bolsa.itens.diamante = 1;
        if (!("esmeralda" in gameglx.perfil.bolsa.itens))
          gameglx.perfil.bolsa.itens.esmeralda = 1;
        if (!("carvao" in gameglx.perfil.bolsa.itens))
          gameglx.perfil.bolsa.itens.carvao = 1;
        if (!("ouro" in gameglx.perfil.bolsa.itens))
          gameglx.perfil.bolsa.itens.ouro = 1;
        if (!("quartzo" in gameglx.perfil.bolsa.itens))
          gameglx.perfil.bolsa.itens.quartzo = 1;
        // Bolsa - naves
        if (!("naves" in gameglx.perfil.bolsa)) gameglx.perfil.bolsa.naves = {};
        if (!("compradas" in gameglx.perfil.bolsa.naves))
          gameglx.perfil.bolsa.naves.compradas = [];
        if (!("status" in gameglx.perfil.bolsa.naves))
          gameglx.perfil.bolsa.naves.status = false;
        // Função de ataque
        if (!("ataque" in gameglx.perfil)) gameglx.perfil.ataque = null;
        if (gameglx.perfil.ataque === null) gameglx.perfil.ataque = {};
        if (!("sendoAtacado" in gameglx.perfil.ataque))
          gameglx.perfil.ataque.sendoAtacado = {};
        if (!("status" in gameglx.perfil.ataque.sendoAtacado))
          gameglx.perfil.ataque.sendoAtacado.status = false;
        if (!("atacante" in gameglx.perfil.ataque.sendoAtacado))
          gameglx.perfil.ataque.sendoAtacado.atacante = null;
        if (!("forcaAtaque" in gameglx.perfil.ataque))
          gameglx.perfil.ataque.forcaAtaque = {};
        if (!("ataque" in gameglx.perfil.ataque.forcaAtaque))
          gameglx.perfil.ataque.forcaAtaque.ataque = 10;
        if (!("data" in gameglx.perfil.ataque)) gameglx.perfil.ataque.data = {};
        if (!("dia" in gameglx.perfil.ataque.data))
          gameglx.perfil.ataque.data.dia = 0;
        if (!("hora" in gameglx.perfil.ataque.data))
          gameglx.perfil.ataque.data.hora = 0;
        if (!("contagem" in gameglx.perfil.ataque.data))
          gameglx.perfil.ataque.data.contagem = 0;
        // Defesa
        if (!("defesa" in gameglx.perfil)) gameglx.perfil.defesa = {};
        if (!("forca" in gameglx.perfil.defesa))
          gameglx.perfil.defesa.forca = 100;
        if (!("ataque" in gameglx.perfil.defesa))
          gameglx.perfil.defesa.ataque = 40;
      } else {
    ...
    const isPrems =
      ...
      isOwner ||
      isMods ||
      global.db.data.users[m.sender].premiumTime > 0; // || global.db.data.users[m.sender].premium = 'true'
    ...
    for (const name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        if (
          ...
          if (
            !["owner-unbanchat.js", "info-creator.js"].includes(name) &&
            chat &&
            chat?.isBanned &&
            !isROwner
          )
...
${tradutor.texto1[1]} ${messageNumber}/3
 ...
 ${tradutor.texto1[4]}`.trim();
          ...
          if (
            ...
            m.text &&
            user &&
            user.lastCommandTime &&
            Date.now() - user.lastCommandTime < 5000 &&
            !isROwner
          ) {
            if (user.commandCount === 2) {
              const remainingTime = Math.ceil(
                (user.lastCommandTime + 5000 - Date.now()) / 1000,
              );
              if (remainingTime > 0) {
        ...
        if (
          ...
          mconn.conn.reply(
            m.chat,
            `${tradutor.texto2} _${usedPrefix}buyall_`,
            m,
          );
          continue;
        }
        if (plugin.level > _user.level) {
          mconn.conn.reply(
            m.chat,
            `${tradutor.texto3[0]} ${plugin.level} ${tradutor.texto3[1]} ${_user.level}, ${tradutor.texto3[2]} ${usedPrefix}lvl ${tradutor.texto3[3]}`,
            m,
          );
          continue;
        }
        ...
        try {
          ...
          if (m.limit) {
            m.reply(
              `${tradutor.texto4[0]} ` + +m.limit + ` ${tradutor.texto4[1]}`,
            );
          }
        }
        break;
  ...
  switch (action) {
    ...
    case "remove":
      if (chat.welcome && !chat?.isBanned) {
        const groupMetadata =
          (await m.conn.groupMetadata(id)) || (conn.chats[id] || {}).metadata;
        for (const user of participants) {
          let pp =
            "https://raw.githubusercontent.com/BrunoSobrino/TheMystic-Bot-MD/master/src/avatar_contact.png";
          try {
            ...
            const userPrefix = antiArab.some((prefix) =>
              user.startsWith(prefix),
            );
            const botTt2 =
              groupMetadata.participants.find(
                (u) => m.conn.decodeJid(u.id) == m.conn.user.jid,
              ) || {};
            ...
            if (
              ...
              const responseb = await m.conn.groupParticipantsUpdate(
                id,
                [user],
                "remove",
              );
            ...
            await m.conn.sendFile(id, apii.data, "pp.jpg", text, null, false, {
              mentions: [user],
            });
          }
        }
    ...
    if (nk.isGroup == false) {
      if (nk.status == "offer") {
        const callmsg = await mconn.conn.reply(
          nk.from,
          `Hola *@${nk.from.split("@")[0]}*, las ${nk.isVideo ? "videollamadas" : "llamadas"} no están permitidas, serás bloqueado.\n-\nSi accidentalmente llamaste póngase es contacto con mi creador para que te desbloquee!`,
          false,
          { mentions: [nk.from] },
        );
...
export async function deleteUpdate(message) {
  ...
  let date = d.toLocaleDateString("ar", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  let time = d.toLocaleString("ar", {
    hour: "numeric",
    minute: "numeric",
    second: "numeric",
    hour12: true,
  ...
  const prep = generateWAMessageFromContent(
    m.chat,
    {
      extendedTextMessage: {
        ...
        contextInfo: {
          externalAdReply: {
            title: tradutor.texto11[0],
            body: tradutor.texto11[1],
            thumbnail: imagen1,
            sourceUrl: tradutor.texto11[2],
          },
        },
      },
    },
